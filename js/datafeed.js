...
Datafeeds.UDFCompatibleDatafeed = function(datafeedURL, userHash, tradingViewClient) {
    this._datafeedURL = datafeedURL;
    this._userHash = userHash;
    this._tradingViewClient = tradingViewClient;
    ...
    this._supported_resolutions_platform = {
        "1":"M1",
        "5":"M5",
        "15":"M15",
        "30":"M30",
...
Datafeeds.UDFCompatibleDatafeed.prototype.getServerTime = function(callback) {
    if (this._configuration.supports_time) {
        this._send(this._datafeedURL + '/time', {user:this._userHash})
...
Datafeeds.UDFCompatibleDatafeed.prototype._initialize = function() {
    ...
    this._send(this._datafeedURL + '/config', {
            user              : this._userHash,
            tradingViewClient : this._tradingViewClient
        })
        .done(function(response) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
    ...
    if (!this._configuration.supports_group_request) {
        this._send(this._datafeedURL + this._symbolResolveURL, {
            ...
            symbolId          : App.tabsBlock.searchSymbolId(symbolName),
            user              : this._userHash,
            tradingViewClient : this._tradingViewClient,
        })
            .done(function(response) {
...
//     this._datafeed._websockets_connect.on('candleLast', dataLastCandle);
//     function dataLastCandle(data){
//
// 		if (data.s === 'ok') {
...
Datafeeds.DataPulseUpdater = function(datafeed) {
    ...
    var that = this;
    App.socketCfd.subscribe('candlestick_feed', updateBars, {}, 'candleTicks');
    ...
    function updateBars(bar){
        ...
        var corrected = correctCandleToLastPoint({
            'low'   : bar.min_value,
            'high'  : bar.max_value,
            'close' : bar.last_value
        });
...
Datafeeds.DataPulseUpdater.prototype.subscribeDataListener = function(symbolInfo, resolution, newDataCallback, listenerGUID) {
    ...
    var infoSymbol = {
        asset_id: parseInt(symbolInfo.id),
        candlestick_name: this._datafeed._supported_resolutions_platform[resolution]
    };

    this._subscribers[listenerGUID].listeners.push(newDataCallback);

    if(App.socketCfd.connectionStatus)
    {
        App.socketCfd.subscribe('candlestick_feed', function(){}, infoSymbol, 'last_point_chart');
    }else{
        App.socketCfd.subscribe('candlestick_feed', function(){}, infoSymbol, 'last_point_chart');
    }
};
...
// 		};
// 	}
// 	this._websockets_connect.lastPoint(symbols);
// 	this._subscribers[listenerGUID].listeners.push(newDataCallback);
// };
...
function correctCandleToLastPoint(data) {

    if (App.asset.currentAssetId === null) {
        return data;
    }

    var useRateParam       = App.sysParameter.USE_RATE_FOR_CHART,
        currentRate        = App.asset.getAssetRates(App.asset.currentAssetId, true),
        proposedChartValue = (useRateParam == '0')
            ? (currentRate.sell_value + currentRate.buy_value)/2
            : (useRateParam == '1'
                ? currentRate.buy_value
                : currentRate.sell_value);

    data.close = proposedChartValue;
    data.low   = proposedChartValue < data.low
        ? proposedChartValue
        : data.low;
    data.high  = proposedChartValue > data.high
        ? proposedChartValue
        : data.high;
